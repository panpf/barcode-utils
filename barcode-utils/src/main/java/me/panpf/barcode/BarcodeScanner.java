...
public class BarcodeScanner {
     ...
     * 启动扫描
     *
     * @throws java.lang.IllegalStateException 已经释放了
     */
    public void start() {
        if (!released) {
            if (!scanning) {
     ...
     */
    public void setDebugMode(@SuppressWarnings("SameParameterValue") boolean debugMode) {
    ...
    private Map<DecodeHintType, Object> createHints(Map<DecodeHintType, Object> hints) {
        ...
        if (!hints.containsKey(DecodeHintType.POSSIBLE_FORMATS)) {
            Vector<BarcodeFormat> decodeFormats = new Vector<>(3);
    ...
    public interface BarcodeScanCallback {
         ...
         *
         * @param result          条码
         * @param bitmapByteArray 当前帧的 bitmap 数据
         * @param scaleFactor     缩放倍数
         * @return true：继续扫描
         */
        boolean onDecodeCallback(Result result, byte[] bitmapByteArray, float scaleFactor);
    }
}
